...
def imagePrep(path):
    ...
    enhancer = ImageEnhance.Brightness(imgOriginal)
    enhancer2= ImageEnhance.Contrast(imgOriginal)
    imgOriginal = enhancer.enhance(1.1)
    imgOriginal = enhancer2.enhance(1.5)
    ...
    imgNums = imgOriginal.crop((600,330+top,1450,top+365))
    ...
    playerResult = reader.readtext(r'C:\Users\Hollis\Desktop\scoreboardcropped.png', detail = 0, text_threshold= .5, low_text=.001, contrast_ths=1)
    playerResult = playerResult[0].split()
    playerResult = [float(i) for i in playerResult]
    print(playerResult)
    columnsResult = reader.readtext(r'C:\Users\Hollis\Desktop\scoreboardcroppedcolumns.png', detail = 0, text_threshold= .9, low_text=.3)
    columnsResult = [i.split() for i in columnsResult]
    columnsResult = [i for j in columnsResult for i in j if i.strip().isalpha()]
    print(columnsResult)

    statColumns = ['DATE', 'SCORE', 'KILLS', 'DEATHS', 'CONFIRMS', 'DENIES', 'TIME', 'ASSISTS', 'LATENCY', 'PLANTS', 'DEFUSES']
 
    masterData = pd.read_csv(r'C:\Users\Hollis\Documents\CodStats\codStats.csv')
    ...
    masterData.to_csv(r'C:\Users\Hollis\Documents\CodStats\codStats.csv', index=False)

    print(masterData)
    
if __name__ == '__main__':
    grab = pg.screenshot()
    grab.save(r'C:\Users\Hollis\Desktop\tmp.png')
    imagePrep(r'C:\Users\Hollis\Desktop\tmp.png')


    print(sum(i for i in range(3,int(input())*3,3)if iin(i*5,i*7)))